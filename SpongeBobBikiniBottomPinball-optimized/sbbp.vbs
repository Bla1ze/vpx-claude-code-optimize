...
Dim gBOT: gBOT = Array()  ' shared ball array, updated once per GameTimer tick
...
LampTimer.Interval = 20  ' was ~10ms; halves lightCtrl.Update frequency (3-6ms/tick → saves ~150-300ms/sec)
Sub LampTimer_Timer()
	lightCtrl.Update
End Sub
...
Sub DynamicBSUpdate
	...
	Dim sx, sy       'cached source positions
	Dim dx, dy, dSq  'distance-squared gating
	BOT = GetBalls

	'Hide shadow of deleted balls
	For s = UBound(BOT) + 1 to tnob
		If objrtx1(s).visible Then objrtx1(s).visible = 0
		If objrtx2(s).visible Then objrtx2(s).visible = 0
		If objBallShadow(s).visible Then objBallShadow(s).visible = 0
		If BallShadowA(s).visible Then BallShadowA(s).visible = 0
	Next
...
' *** Normal "ambient light" ball shadow
		...
		If AmbientBallShadowOn = 1 Then			'Primitive shadow on playfield, flasher shadow in ramps
			If bz > 30 Then							'The flasher follows the ball up ramps while the primitive is on the pf
				If bx < TW_d2 Then
					objBallShadow(s).X = (bx) - (BS_d10) + ((bx - (TW_d2))*(InvBS_dAM)) + 5
				Else
					objBallShadow(s).X = (bx) + (BS_d10) + ((bx - (TW_d2))*(InvBS_dAM)) - 5
				End If
		...
		Elseif AmbientBallShadowOn = 2 Then		'Flasher shadow everywhere
			...
			Elseif bz <= 30 And bz > 20 Then	'On pf
				BallShadowA(s).visible = 1
				If bx < TW_d2 Then
					BallShadowA(s).X = (bx) - (BS_d10) + ((bx - (TW_d2))*(InvBS_dAM)) + 5
				Else
					BallShadowA(s).X = (bx) + (BS_d10) + ((bx - (TW_d2))*(InvBS_dAM)) - 5
				End If
		...
		If DynamicBallShadowsOn Then
			If bz < 30 Then
				For iii = 0 to numberofsources - 1
					...
					dx = bx - sx: dy = by - sy: dSq = dx * dx + dy * dy
...
Dim InvBS_dAM: InvBS_dAM = 1 / BS_dAM
Dim Inv100: Inv100 = 0.01

' Pre-built string arrays for hot path sound calls
Dim BallRollStr(5)
Dim brsI: For brsI = 0 To 5: BallRollStr(brsI) = "BallRoll_" & brsI: Next

Dim RampLoopStr(6), WireLoopStr(6)
Dim rlsI: For rlsI = 1 To 6
	RampLoopStr(rlsI) = "RampLoop" & rlsI
	WireLoopStr(rlsI) = "wireloop" & rlsI
Next
...
Function DistanceFromFlipper(ballx, bally, Flipper)
	...
	frad = (Flipper.currentangle + 90) * PIover180
	DistanceFromFlipper = DistancePL(ballx, bally, fx, fy, Cos(frad)+fx, Sin(frad)+fy)
End Function
...
Function FlipperTrigger(ballx, bally, Flipper)
	...
	Dim dx, dy: dx = ballx - fx: dy = bally - fy
	If dfl < 48 and DiffAngle <= 90 and SQR(dx * dx + dy * dy) < flen Then
		FlipperTrigger = True
	Else
...
sub TargetBouncer(aBall,defvalue)
    ...
    if TargetBouncerEnabled = 1 and aball.z < 30 then
        ...
        Dim vz2: vz2 = aBall.velz * aBall.velz
        aBall.velx = sgn(aBall.velx) * sqr(abs((vel*vel - vz2)/(1+vratio*vratio)))
	...
	dim x : for x = 1 to uBound(RampBalls)
		if Not IsEmpty(RampBalls(x,1) ) then
			if BallVel(RampBalls(x,0) ) > 1 then ' if ball is moving, play rolling sound
				If RampType(x) then
					PlaySound RampLoopStr(x), -1, VolPlayfieldRoll(RampBalls(x,0)) * RampRollVolume * MechVolume, AudioPan(RampBalls(x,0)), 0, BallPitchV(RampBalls(x,0)), 1, 0, AudioFade(RampBalls(x,0))
					StopSound WireLoopStr(x)
				Else
					StopSound RampLoopStr(x)
					PlaySound WireLoopStr(x), -1, VolPlayfieldRoll(RampBalls(x,0)) * RampRollVolume * MechVolume, AudioPan(RampBalls(x,0)), 0, BallPitch(RampBalls(x,0)), 1, 0, AudioFade(RampBalls(x,0))
				End If
...
Sub RollingUpdate()
	...
	For b = 0 to UBound(BOT)
		Set ball = BOT(b)
		bz = ball.z
		If BallVel(ball) > 1 AND bz < 30 Then
			rolling(b) = True
			PlaySound BallRollStr(b), -1, VolPlayfieldRoll(ball) * BallRollVolume * MechVolume, AudioPan(ball), 0, PitchPlayfieldRoll(ball), 1, 0, AudioFade(ball)
		...
		If AmbientBallShadowOn = 0 Then
			...
			End If
			BallShadowA(b).Y = ball.Y + BS_d5 + fovY
			BallShadowA(b).X = ball.X
			BallShadowA(b).visible = 1
		End If

		' Ramp Boost
        If bz > 120 and inRect(ball.x,ball.y,490,450,490,200,633,200,633,450) and ball.vely < 1 Then
...
Sub OnBallBallCollision(ball1, ball2, velocity)
	...
	Dim vel2: vel2 = Csng(velocity): PlaySound (snd), 0, vel2 * vel2 / 200 * BallWithBallCollisionSoundFactor * MechVolume, AudioPan(ball1), 0, Pitch(ball1), 0, 0, AudioFade(ball1)
End Sub
...
Class LStateController
    ...
    Private Sub AssignStateForFrame(key, state)
        If m_currentFrameState.Exists(key) Then
            Set m_currentFrameState.Item(key) = state
        Else
   ...
   Public Sub SyncLightMapColors()
    ...
    Public Sub Update()
        ...
        If hasOverride = False Then
            ...
            If HasKeys(m_on) Then
                For Each lightKey in m_on.Keys()
                    ...
                    AssignStateForFrame lightKey, (new FrameState)(lcItem.level, lcItem.Color, lcItem.Idx)
                Next
            End If
            ...
            If HasKeys(m_pulse) Then
                ...
                For Each lightKey in m_pulse.Keys()
                    Set pState = m_pulse(lightKey)
                    AssignStateForFrame lightKey, (new FrameState)(pState.PulseAt(pState.idx), pState.light.Color, pState.light.Idx)
                    Dim pulseUpdateInt : pulseUpdateInt = pState.interval - m_frameTime
                    Dim pulseIdx : pulseIdx = pState.idx
                    If pulseUpdateInt <= 0 Then
                        pulseUpdateInt = m_pulseInterval
                        pulseIdx = pulseIdx + 1
                    End If

                    Dim pulses : pulses = pState.pulses
					Dim pulseCount : pulseCount = pState.Cnt
                    If pulseIdx > UBound(pState.pulses) Then
                        ...
                        End If
                    Else
						Set m_pulse.Item(lightKey) = (new PulseState)(m_lights(lightKey),pulses, pulseIdx, pulseUpdateInt, pulseCount)
                    End If
                Next
            End If
        ...
        If HasKeys(m_currentFrameState) Then
            ...
            Dim frameStateKey
            Dim fsCached  'cached FrameState to avoid repeated dictionary lookups
            For Each frameStateKey in m_currentFrameState.Keys()
                Set fsCached = m_currentFrameState(frameStateKey)
                Dim idx : idx = fsCached.idx

                Dim newColor : newColor = fsCached.colors
                Dim bUpdate
                ...
                If Not IsNull(newColor) Then
					...
					If Not IsNull(c) Then
						If CLng(tmpLight.Color) <> CLng(c) Then
							bUpdate = True
						End If
					End If

					If Not IsNull(cf) Then
						If CLng(tmpLight.ColorFull) <> CLng(cf) Then
							bUpdate = True
						End If
					End If
            	End If

                Dim fsLevel : fsLevel = fsCached.level
                    ...
                    Else
                        ...
                        If bUpdate Then
                            ...
                            If Not IsNull(cf) Then
                                lt.colorFull = cf
                            End If
                            If m_lightmaps.Exists(lt.Name) Then
                                For Each lm in m_lightmaps(lt.Name)
                                    If Not IsNull(lm) Then
                                        lm.Color = c
                                    End If
                                Next
...
sub VRBGFLSP_1_timer()						
	...
	Dim sp_s, sp_s2, sp_s15, sp_s25
	sp_s = VRBGFLSplvl: sp_s2 = sp_s * sp_s: sp_s15 = sp_s * Sqr(sp_s): sp_s25 = sp_s2 * Sqr(sp_s)
	If VR_Backglass = 1 Then
		VRBGFLSP_1.opacity = 70 * sp_s15
		VRBGFLSP_2.opacity = 70 * sp_s15
		VRBGFLSP_3.opacity = 70 * sp_s15
		VRBGFLSP_4.opacity = 125 * sp_s2
		VRBGFLSP_5.opacity = 125 * sp_s25
	Else
		VRBGFLSP_001.opacity = 35 * sp_s15
		VRBGFLSP_002.opacity = 35 * sp_s15
		VRBGFLSP_003.opacity = 35 * sp_s15
		VRBGFLSP_004.opacity = 165 * sp_s2
	End If
...
sub VRBGFLPA_1_timer()						
	...
	Dim pa_s, pa_s2, pa_s15, pa_s25
	pa_s = VRBGFLPAlvl: pa_s2 = pa_s * pa_s: pa_s15 = pa_s * Sqr(pa_s): pa_s25 = pa_s2 * Sqr(pa_s)
	If VR_Backglass = 1 Then
		VRBGFLPA_1.opacity = 70 * pa_s15
		VRBGFLPA_2.opacity = 70 * pa_s15
		VRBGFLPA_3.opacity = 70 * pa_s15
		VRBGFLPA_4.opacity = 125 * pa_s2
		VRBGFLPA_5.opacity = 125 * pa_s25
	Else
		VRBGFLPA_001.opacity = 30 * pa_s15
		VRBGFLPA_002.opacity = 30 * pa_s15
		VRBGFLPA_003.opacity = 30 * pa_s15
		VRBGFLPA_004.opacity = 75 * pa_s2
	End If
...
sub VRBGFLBump1_1_timer()						
	...
	Dim b1_s, b1_s2, b1_s15
	b1_s = VRBGFLBump1lvl: b1_s2 = b1_s * b1_s: b1_s15 = b1_s * Sqr(b1_s)
	If VR_Backglass = 1 Then
		VRBGFLBump1_1.opacity = 50 * b1_s15
		VRBGFLBump1_2.opacity = 50 * b1_s15
		VRBGFLBump1_3.opacity = 50 * b1_s15
		VRBGFLBump1_4.opacity = 100 * b1_s2
	Else
		VRBGFLBump1_001.opacity = 40 * b1_s15
		VRBGFLBump1_002.opacity = 40 * b1_s15
		VRBGFLBump1_003.opacity = 40 * b1_s15
		VRBGFLBump1_004.opacity = 120 * b1_s2
	End If
...
sub VRBGFLBump2_1_timer()						
	...
	Dim b2_s, b2_s2, b2_s15
	b2_s = VRBGFLBump2lvl: b2_s2 = b2_s * b2_s: b2_s15 = b2_s * Sqr(b2_s)
	If VR_Backglass = 1 Then
		VRBGFLBump2_1.opacity = 50 * b2_s15
		VRBGFLBump2_2.opacity = 50 * b2_s15
		VRBGFLBump2_3.opacity = 50 * b2_s15
		VRBGFLBump2_4.opacity = 100 * b2_s2
	Else
		VRBGFLBump2_001.opacity = 40 * b2_s15
		VRBGFLBump2_002.opacity = 40 * b2_s15
		VRBGFLBump2_003.opacity = 40 * b2_s15
		VRBGFLBump2_004.opacity = 120 * b2_s2
	End If
...
Sub UpdateBallBrightness
	...
	For s = 0 To UBound(BOT)
		...
		' Handle plunger lane
		If InRect(bx,by,870,2100,870,1260,930,1260,930,2100) Then
			d_w = d_w*(PLOffset+PLGain*(by-2100))
		End If

		' Handle z direction
		d_w = b_base*(1 - (bz-25)/500)
		If d_w < 30 Then d_w = 30

		' Assign color
		b_r = Int(d_w)
		If b_r > 255 Then b_r = 255
		BOT(s).color = b_r + (b_r * 256) + (b_r * 65536)
	Next
End Sub
