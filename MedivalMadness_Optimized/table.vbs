...
' Pre-computed BallSize constants
Dim BS_d4 : BS_d4 = BallSize / 4
Dim BS_d2 : BS_d2 = BallSize / 2
Dim BS_d5 : BS_d5 = BallSize / 5

' Pre-built string arrays to eliminate per-tick string concatenation
Dim BallRollStr(5), RampLoopStr(6), WireLoopStr(6)
Dim brIdx
For brIdx = 0 To 5
...
Dim rlIdx
For rlIdx = 0 To 6
	RampLoopStr(rlIdx) = "RampLoop" & rlIdx
	WireLoopStr(rlIdx) = "wireloop" & rlIdx
Next
...
Dim FVU_lastL, FVU_lastR
Sub FlipperVisualUpdate
	Dim lca : lca = LeftFlipper.CurrentAngle
	Dim rca : rca = RightFlipper.CurrentAngle
	If lca <> FVU_lastL Then FlipperLSh.RotZ = lca : FVU_lastL = lca
	If rca <> FVU_lastR Then FlipperRSh.RotZ = rca : FVU_lastR = rca
End Sub
...
Function DistanceFromFlipper(ballx, bally, Flipper)
	...
	rad = Radians(Flipper.currentangle+90)
...
Sub RollingUpdate()
	...
	Dim curBall, vx, vy, bvel, bx, by, bz, vz
