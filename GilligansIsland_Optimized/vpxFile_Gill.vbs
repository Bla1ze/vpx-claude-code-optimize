...
Dim FlipperActive
FlipperActive = False
Dim tablewidth: tablewidth = Gilligan.width
Dim tableheight: tableheight = Gilligan.height
Dim lastLFAngle, lastRFAngle
Dim lastVUKGateAngle, lastGate4Angle
...
Sub GateTimer_Timer
	Dim g
	g = VUKGate.Currentangle
	If g <> lastVUKGateAngle Then lastVUKGateAngle = g : Primitive_VUKGateWire.RotX = -g
	g = Gate4.Currentangle
	If g <> lastGate4Angle Then lastGate4Angle = g : Primitive_GateWire.RotX = -g
End Sub
...
Dim brsI : For brsI = 0 To tnob : BallRollStr(brsI) = "fx_ballrolling" & brsI : BallDropStr(brsI) = "fx_ball_drop" & brsI : Next
InitRolling
...
Sub RollingTimer_Timer()
    Dim BOT, b, bx, by, bz, bvx, bvy, bvz, bvel
    BOT = GetBalls
    ...
    For b = 0 to UBound(BOT)
        ...
        If bvel > 1 Then
            ...
            If bz < 30 Then ' Ball on playfield
                PlaySound BallRollStr(b), -1, Csng(bvel * bvel / 400)/12, AudioPanXY(bx), 0, bvel * 20, 1, 0, AudioFadeXY(by)
            Else ' Ball on raised ramp
                PlaySound BallRollStr(b), -1, Csng(bvel * bvel / 400)/12, AudioPanXY(bx), 0, bvel * 20+50000, 1, 0, AudioFadeXY(by)
            End If
        Else
        ...
        If bvz < -1 And bz < 55 And bz > 27 Then 'height adjust for ball drop sounds
            PlaySound BallDropStr(b), 0, ABS(bvz)/17, AudioPanXY(bx), 0, bvel * 20, 1, 0, AudioFadeXY(by)
        End If
    Next
End Sub
...
Sub OnBallBallCollision(ball1, ball2, velocity)
	...
	PlaySound "fx_collide", 0, cv * cv / 2000, AudioPan(ball1), 0, Pitch(ball1), 0, 0, AudioFade(ball1)
End Sub
