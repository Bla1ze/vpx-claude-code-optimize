...
Sub FlipperVisualUpdate
	...
	g = Gate1.CurrentAngle : If g <> lastGate1Angle Then lastGate1Angle = g : Gate1p.rotx = max(g,0)
	g = Gate2.CurrentAngle : If g <> lastGate2Angle Then lastGate2Angle = g : Gate2p.rotx = max(g,0)
	g = Gate3.CurrentAngle : If g <> lastGate3Angle Then lastGate3Angle = g : Gate3p.rotx = max(g,0)
	g = Gate4.CurrentAngle : If g <> lastGate4Angle Then lastGate4Angle = g : Gate4p.rotx = max(g,0)
End Sub
...
Sub RollingUpdate()
	Dim b, bx, by, bz, bvx, bvy, bvz, bvel
	...
	For b = 0 to UBound(gBOT)
		...
		If bvel > 1 AND bz < 30 Then
			rolling(b) = True
			PlaySound BallRollStr(b), -1, RollingSoundFactor * 0.0005 * (bvel * bvel * bvel) * BallRollVolume * VolumeDial, AudioPanXY(bx), 0, bvel * bvel * 15, 1, 0, AudioFadeXY(by)
...
'		' "Static" Ball Shadows
		If AmbientBallShadowOn = 0 Then
			...
			If bz > 30 Then
				BallShadowA(b).height = bz - BallSize/4 + b/1000	'This is technically 1/4 of the ball "above" the ramp, but it keeps it from clipping the ramp
