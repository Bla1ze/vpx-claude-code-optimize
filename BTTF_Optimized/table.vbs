...
if DesktopMode = False Then Ramp3.visible = 0:Ramp4.visible = 0
...
Const UseFlexDMD = 0	' 1 is on to replace external dmd.
...
' --- PRE-CACHED TABLE DIMENSIONS (eliminates 2 COM reads per Pan/AudioFade call) ---
Dim tablewidth : tablewidth = bttf.width
Dim tableheight : tableheight = bttf.height

' --- PRE-COMPUTED TRIG CONSTANTS ---
Dim DEG2RAD : DEG2RAD = 2 * 3.14159265358979 / 180
Dim DEG2RAD_HALF : DEG2RAD_HALF = 2 * 3.14159265358979 / 360

' --- PRE-BUILT SOUND STRINGS (eliminates string concatenation in CollisionTimer) ---
Dim RollStr(7), MetalRollStr(7)
Dim iStr
For iStr = 0 To 7
    RollStr(iStr) = "fx_ballrolling" & iStr
    MetalRollStr(iStr) = "metalrolling" & iStr
Next

' --- HOISTED BALLSHADOW ARRAY (initialized in bttf_Init) ---
Dim BallShadow(6)

' --- PRE-CACHED RAMP WEDGE POSITIONS (static prims, positions never change) ---
Dim rw1x, rw1y, rw2x, rw2y
Dim rw1ax, rw1ay, rw1bx, rw1by, rw1cx, rw1cy, rw1dx, rw1dy
Dim rw2ax, rw2ay, rw2bx, rw2by, rw2cx, rw2cy, rw2dx, rw2dy
...
Sub bttf_Init
    ...
    With Controller
		...
		If DesktopMode = true then .hidden = 1 Else .hidden = 1 End If
        .Run GetPlayerHWnd
...
' ball through system
    ...
    Set BallShadow(6) = BallShadow7

    ' Cache static ramp wedge positions for InRect exclusion
    rw1x = pRampWedge1.x : rw1y = pRampWedge1.y
    rw2x = pRampWedge2.x : rw2y = pRampWedge2.y
    rw1ax = rw1x - 40 : rw1ay = rw1y - 40 : rw1bx = rw1x + 40 : rw1by = rw1y - 40
    rw1cx = rw1x + 40 : rw1cy = rw1y + 40 : rw1dx = rw1x - 40 : rw1dy = rw1y + 40
    rw2ax = rw2x - 40 : rw2ay = rw2y - 40 : rw2bx = rw2x + 40 : rw2by = rw2y - 40
    rw2cx = rw2x + 40 : rw2cy = rw2y + 40 : rw2dx = rw2x - 40 : rw2dy = rw2y + 40
...
Sub MyTimer_Timer()
    ...
    ' Cache gate angles (each read once instead of 2×)
    Dim g1a : g1a = gate1.CurrentAngle
    Dim g2a : g2a = gate2.CurrentAngle
    Dim g3a : g3a = gate3.CurrentAngle
    Dim g4a : g4a = gate4.CurrentAngle

    p_gate1.Rotx = g1a + 120
    p_gate2.Rotx = g2a
    p_gate3.Rotx = g3a

	Gate3Angle = Int(g3a)
	If Gate3Angle > 0 then
	pGate3_switch.ObjRotY = sin( Gate3Angle * DEG2RAD) * 10
	Else
	pGate3_switch.ObjRotY = sin( (-Gate3Angle) * DEG2RAD) * 10
	End If
	...
	pGate4_switch.ObjRotY = sin( (-Gate4Angle) * DEG2RAD) * 10
	Else
	pGate4_switch.ObjRotY = sin( Gate4Angle * DEG2RAD) * 10
	End If
    ...
    ' Cache flipper angles (each read once instead of 2-3×)
    Dim lfA : lfA = LeftFlipper.Currentangle
    Dim rfA : rfA = RightFlipper.Currentangle

	pLeftFlipperLogo.Roty = lfA
'	pLSS.Roty = lfA - 90
	FlipperLSh.RotZ = lfA

	pRightFlipperLogo.Roty = rfA
	pRSS.Roty = rfA - 90
	FlipperRSh.RotZ = rfA

    ' Cache spinner angle (read once instead of 3×)
    Dim spA : spA = sw28.Currentangle
	pSpinner.RotX = spA * -1

	pSpinnerRod.TransX = sin( (spA + 180) * DEG2RAD_HALF) * 5
	pSpinnerRod.TransY = sin( (spA - 90) * DEG2RAD_HALF) * 5
...
Sub LeftSlingShot_Slingshot:LeftSlingshota.visible = false:pSlingL.TransZ = -8:LeftSlingshotb.visible = true:PlaySoundAt SoundFX("slingshot_l",DOFContactors),pSlingL:vpmTimer.PulseSw 21:LeftSlingshotStep = 0:Me.TimerEnabled = 1:If WobblePlastic = 1 then:PlasticWobbling = 1:WobbleCount = 2:tWobblePlastic.Enabled = True:End If:End Sub
...
Sub RightSlingShot_Slingshot:RightSlingshota.visible = false:pSlingR.TransZ = -8:RightSlingshotb.visible = true:PlaySoundAt SoundFX("slingshot_r",DOFContactors),pSlingR:vpmTimer.PulseSw 22:RightSlingshotStep = 0:Me.TimerEnabled = 1:If WobblePlastic = 1 then:PlasticWobbling = 1:WobbleCount = 2:tWobblePlastic.Enabled = True:End If:End Sub
...
' Bumpers
Sub Bumper1_Hit:vpmTimer.PulseSw 46:PlaySoundAtBumperVol SoundFX("bumper1",DOFContactors),Bumper1,1:bump1 = 1:Me.TimerEnabled = 1:If WobblePlastic = 1 then:PlasticWobbling = 1:WobbleCount = 2:tWobblePlastic.Enabled = True:End If::End Sub
Sub Bumper1_Timer()
    Select Case bump1
        Case 1:
        Case 2:
...
Sub Bumper2_Hit:vpmTimer.PulseSw 47:PlaySoundAtBumperVol SoundFX("bumper2",DOFContactors),Bumper2,1:bump2 = 1:Me.TimerEnabled = 1:If WobblePlastic = 1 then:PlasticWobbling = 1:WobbleCount = 2:tWobblePlastic.Enabled = True:End If:End Sub
Sub Bumper2_Timer()
    Select Case bump2
        Case 1:
        Case 2:
...
Sub Bumper3_Hit:vpmTimer.PulseSw 48:PlaySoundAtBumperVol SoundFX("bumper1",DOFContactors),Bumper3,1:bump3 = 1:Me.TimerEnabled = 1:If WobblePlastic = 1 then:PlasticWobbling = 1:WobbleCount = 2:tWobblePlastic.Enabled = True:End If:End Sub
Sub Bumper3_Timer()
    Select Case bump3
        Case 1:
        Case 2:
...
Sub sw45_Hit:bsTR.AddBall 0:Psw45.TransY = -5:PlaySoundAtVol "solenoid2",sw45,2:End Sub
...
Sub SolLFlipper(Enabled)
    If Enabled Then
		PlaySoundAt SoundFX("flipperup_l",DOFFlippers),LeftFlipper:LeftFlipper.RotateToEnd
    Else
...
Sub SolRFlipper(Enabled)
    If Enabled Then
        PlaySoundAt SoundFX("flipperup_r",DOFFlippers),RightFlipper:RightFlipper.RotateToEnd
    Else
...
reDim CollapseMe(3) 'LampTimer  (Click Me to Collapse)
    ...
    Sub LampTimer_Timer()
        ...
        Dim chgLamp, num, chg, ii, lampId, lampVal
        chgLamp = Controller.ChangedLamps
        If Not IsEmpty(chgLamp) Then
            For ii = 0 To UBound(chgLamp)
                lampId = chgLamp(ii, 0)
                lampVal = chgLamp(ii, 1)
                LampState(lampId) = lampVal
                FadingLevel(lampId) = lampVal + 4
            Next
        End If
...
Sub CollisionTimer_Timer()
    ...
    For b = 0 to ub
        ...
        If vel > 1 Then
            rolling(b) = True
            volVal = Csng(velSq / 200) * 1.2
            pitchVal = vel * 20
            If bz < 30 Then ' Ball on playfield
                PlaySound RollStr(b), -1, volVal*.7, panVal, 0, pitchVal, 1, 0, fadeVal
            Else ' Ball on raised ramp
                If OnWireRamp = 1 Then
                    PlaySound MetalRollStr(b), -1, volVal*.5, panVal, 0, pitchVal, 1, 0, fadeVal
                Else
                    PlaySound RollStr(b), -1, volVal*.5, panVal, 0, pitchVal+60000, 1, 0, fadeVal
                End If
            End If
        Else
        ...
        'Ball Drop Hits
        If vz < 0 and bz < 60 and bz > 40 Then
            If InRect(bx,by,rw1ax,rw1ay,rw1bx,rw1by,rw1cx,rw1cy,rw1dx,rw1dy) Then
            ElseIf InRect(bx,by,rw2ax,rw2ay,rw2bx,rw2by,rw2cx,rw2cy,rw2dx,rw2dy) Then
            Else
...
Sub FlexDMD_Init() 'default/startup values
	...
	If Not FlexDMD is Nothing Then
		...
		FlexDMD.TableFile = bttf.Filename & ".vpx"
		...
		With FlexDMDScene
			...
			For i = 0 to 15
				...
				.GetImage("Seg" & cstr(i)).SetAlignedPosition i * 8,1,0
				'line 2
				.AddActor FlexDMD.NewImage("Seg" & cstr(i + 16), "VPX.DMD_Space")
				.GetImage("Seg" & cstr(i + 16)).SetAlignedPosition i * 8,17,0
			Next 

			.AddActor FlexDMD.NewImage("Lightning", "VPX.DMD_Lightning_" & "1")
			.GetImage("Lightning").SetAlignedPosition 0,0,0
			.GetImage("Lightning").Visible = False
...
Sub FlexDictionary_Init
	...
	FlexDMDDict.Add 15, "VPX.DMD_Ddelorean"
	FlexDMDDict.Add 2136, "VPX.DMD_CLower"
...
Sub FlexLightning
	...
	FlexDMDScene.GetImage("Lightning").Bitmap = FlexDMD.NewImage("", "VPX.DMD_Lightning_" & Int(4 * Rnd) + 1).Bitmap
	FlexDMDScene.GetImage("Lightning").SetAlignedPosition Int(100 * Rnd),0,0
	FlexDMDScene.GetImage("Lightning").Visible = True
