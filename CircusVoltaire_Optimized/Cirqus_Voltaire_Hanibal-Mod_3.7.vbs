...
Const dmdcolor = 5 'Set the color of the NORMAL Integrated DMD - '0 = Orange / 1 = Green / 2 = Red / 3 = Pink / 4 = Blue / 5 Cyan / 6 Purple / 7 Yellow
...
Dim PrevGIRightStep, PrevGIMiddleStep, PrevGILeftStep, PrevAllGIState
PrevGIRightStep = -1
PrevGIMiddleStep = -1
PrevGILeftStep = -1
PrevAllGIState = -1
...
Dim PiOver180, DegPerRad, InvTableWidthHalf
PiOver180 = Pi / 180
DegPerRad = 180 / Pi
...
Function Vol(ball) ' Calculates the Volume of the sound based on the ball speed
    Dim speed
    speed = BallVel(ball)
    Vol = Csng((speed * speed) / 200)
End Function

Function Pan(ball) ' Calculates the pan for a ball based on the X position on the table. "table1" is the name of the table
    Dim tmp, absTmp, tmp2, tmp4, tmp8
    tmp = (ball.x * InvTableWidthHalf) - 1
    If tmp > 0 Then
        tmp2 = tmp * tmp
        tmp4 = tmp2 * tmp2
        tmp8 = tmp4 * tmp4
...
Sub OnBallBallCollision(ball1, ball2, velocity)
	Dim velSq, ballPan1, ballPitch1
	velSq = (Csng(velocity) * Csng(velocity)) / 200
	ballPan1 = Pan(ball1)
	ballPitch1 = Pitch(ball1)
	PlaySound("fx_collide"), 0, velSq, ballPan1, 0, ballPitch1, 1, 0
    If ball1.id = 666 OR ball2.id = 666 Then
    PlaySound("rubber_hit_3"), 0, velSq, ballPan1, 0, ballPitch1, 1, 0
    else
	PlaySound("fx_collide"), 0, velSq, ballPan1, 0, ballPitch1, 0, 0
    End If
