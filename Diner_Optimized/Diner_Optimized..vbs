...
Dim lastLeftAngle, lastRightAngle	' cached flipper angles for GraphicsTimer guard writes
...
Sub GraphicsTimer_Timer()
	...
	If ChooseBats = 1 Then
		' *** move primitive bats — guard writes, flippers at rest skips 4 COM writes per frame ***
		...
		End If
		Dim ra : ra = RightFlipper.CurrentAngle
		If ra <> lastRightAngle Then
			lastRightAngle = ra
			batright.objrotz = ra - 1
			batrightshadow.objrotz = ra - 1
		End If
	Else
		If ChooseBats > 1 Then
			' *** move glowbats — guard writes ***
			Dim gla : gla = LeftFlipper.CurrentAngle
			If gla <> lastLeftAngle Then
				lastLeftAngle = gla
				GlowBatLightLeft.y = 1720 - 121 + gla
				glowbatleft.objrotz = gla
			End If
			Dim gra : gra = RightFlipper.CurrentAngle
			If gra <> lastRightAngle Then
				lastRightAngle = gra
				GlowBatLightRight.y = 1720 - 121 - gra
				glowbatright.objrotz = gra
			End If
		End If
	End If
End Sub
...
' Pre-built sound name strings — eliminates "BallRoll_" & b concatenation in RollingUpdate (3x per ball per 10ms tick)
...
Sub RollingUpdate()
	...
	For b = 0 to UBound(BOT)
		bz = BOT(b).z : bx = BOT(b).X : by = BOT(b).Y		'cache COM reads once for all uses below
...
' Pre-computed constants for hot-path loops — eliminates repeated division/exponentiation per frame
Dim DynBSFactor2 : DynBSFactor2 = DynamicBSFactor * DynamicBSFactor   ' replaces DynamicBSFactor^2 (0.9025)
Dim DynBSFactor3 : DynBSFactor3 = DynBSFactor2 * DynamicBSFactor      ' replaces DynamicBSFactor^3 (0.857375)
Dim BS_d10       : BS_d10       = BallSize / 10                        ' BallSize/10 = 5
Dim BS_d5        : BS_d5        = BallSize / 5                         ' BallSize/5  = 10
Dim BS_d4        : BS_d4        = BallSize / 4                         ' BallSize/4  = 12.5
Dim BS_d2        : BS_d2        = BallSize / 2                         ' BallSize/2  = 25
Dim BS_dAM       : BS_dAM       = BallSize / AmbientMovement           ' BallSize/AmbientMovement = 25
Dim TW_d2        : TW_d2        = tablewidth / 2                       ' tablewidth/2
...
Sub DynamicBSUpdate
	...
	Dim s, LSd, currentMat, AnotherSource, BOT, iii
	Dim bx, by, bz, sx, sy, asx, asy		'per-ball and per-source cached COM reads
	BOT = GetBalls
...
'The Magic happens now
	For s = lob to UBound(BOT)
		bx = BOT(s).X : by = BOT(s).Y : bz = BOT(s).Z		'cache 3 COM reads once for all uses below
...
' *** Dynamic shadows
		If DynamicBallShadowsOn Then
			If bz < 30 Then 'And BOT(s).Y < (TableHeight - 200) Then 'Or BOT(s).Z > 105 Then		'Defining when and where (on the table) you can have dynamic shadows
				For iii = 0 to numberofsources - 1
					sx = DSSources(iii)(0) : sy = DSSources(iii)(1)		'cache source coords once per source
					LSd=DistanceFast((bx-sx),(by-sy))	'Calculating the Linear distance to the Source
					If LSd < 150 And gilvl > 0 Then						    'If the ball is within the falloff range of a light and light is on (we will set numberofsources to 0 when GI is off)
	...
	'						objrtx1(s).Z = bz - 25 + s/1000 + 0.01						'Uncomment if you want to add shadows to an upper/lower pf
							...
							ShadowOpacity = (150-LSd)/150									'Sets opacity/darkness of shadow by distance to light
						...
						Elseif currentShadowCount(s) = 2 Then
							...
							asx = DSSources(AnotherSource)(0) : asy = DSSources(AnotherSource)(1)	'cache prior source coords
							objrtx1(s).visible = 1 : objrtx1(s).X = bx : objrtx1(s).Y = by + fovY
	'						objrtx1(s).Z = bz - 25 + s/1000 + 0.01							'Uncomment if you want to add shadows to an upper/lower pf
							objrtx1(s).rotz = AnglePP(asx, asy, bx, by) + 90
							ShadowOpacity = (150-DistanceFast((bx-asx),(by-asy)))/150
...
' Pre-built ramp sound name strings — eliminates "RampLoop"&x / "wireloop"&x concatenation in RampRollUpdate
...
Sub FlashFlasher(nr)
	If not objflasher(nr).TimerEnabled Then
		...
		End Select
	End If
	' cache ObjLevel exponents — eliminates up to 13 repeated power ops per call
	Dim flvl : flvl = ObjLevel(nr)
	Dim flvl2 : flvl2 = flvl * flvl
	Dim flvl3 : flvl3 = flvl2 * flvl
	objflasher(nr).opacity = 1000 * FlasherFlareIntensity * flvl2
	objbloom(nr).opacity = 100 * FlasherBloomIntensity * flvl2
	objlight(nr).IntensityScale = 0.5 * FlasherLightIntensity * flvl3
	objbase(nr).BlendDisableLighting = FlashOffDL + 10 * flvl3
	objlit(nr).BlendDisableLighting = 10 * flvl2
	UpdateMaterial "Flashermaterial" & nr,0,0,0,0,0,0,flvl,RGB(255,255,255),0,0,False,True,0,0,0,0
	Select Case nr
		case 1:
			If VRRoom > 0 Then
				VRBGFL9_1.opacity = 100 * flvl2
				VRBGFL9_2.opacity = 100 * flvl2
				VRBGFL9_3.opacity = 100 * flvl2
				VRBGFL9_4.opacity = 100 * flvl3
			End If
		case 3:
			If VRRoom > 0 Then
				VRBGFL11_1.opacity = 100 * flvl2
				VRBGFL11_2.opacity = 100 * flvl2
				VRBGFL11_3.opacity = 100 * flvl2
				VRBGFL11_4.opacity = 100 * flvl3
			End If
	End Select
...
Sub BeerTimer_Timer()
	' Randomize(21) removed — re-seeding with the same fixed seed every tick was redundant
...
Dim ClkSec, ClkMin, ClkHour	' cached clock state for ClockTimer guard writes
' VR Clock code below....
Sub ClockTimer_Timer()
	' guard writes — clock hands only change once per second at most; avoid 4 Now() + 3 COM writes per tick
	Dim t : t = Now()
	Dim cs : cs = Second(t)
	If cs <> ClkSec Then
		ClkSec = cs
		Pseconds.RotAndTra2 = cs * 6
		Dim cm : cm = Minute(t)
		If cm <> ClkMin Then
			ClkMin = cm
			Pminutes.RotAndTra2 = (cm + (cs / 100)) * 6
			Dim ch : ch = Hour(t)
			If ch <> ClkHour Then
				ClkHour = ch
				Phours.RotAndTra2 = ch * 30 + (cm / 2)
			End If
		End If
	End If
