...
Sub FlashInit
    ...
    For i = 0 to 200
        ...
        FlashLevel(i) = 0
    Next
    FlashSpeedUp = 300   'fast speed when turning on the flasher (scaled 6x for 30ms interval)
    FlashSpeedDown = 60  'slow speed when turning off the flasher (scaled 6x for 30ms interval)
...
Sub DynamicBSUpdate
	...
	Dim bx, by, bz, dx, dy, dsq, sx, sy

	'Hide shadow of deleted balls
	For s = ub + 1 to tnob - 1
		objrtx1(s).visible = 0
		objrtx2(s).visible = 0
	...
	For s = lob to ub
		...
		If AmbientBallShadowOn = 1 Then
			If bz > 30 Then
				If bx < TW_d2 Then
					objBallShadow(s).X = bx - BS_d10 + ((bx - TW_d2) / BS_dAM) + offsetX + 5
				Else
					objBallShadow(s).X = bx + BS_d10 + ((bx - TW_d2) / BS_dAM) + offsetX - 5
				End If
		...
		Elseif AmbientBallShadowOn = 2 Then
			...
			Elseif bz <= 30 And bz > 20 Then
				BallShadowA(s).visible = 1
				If bx < TW_d2 Then
					BallShadowA(s).X = bx - BS_d10 + ((bx - TW_d2) / BS_dAM) + offsetX + 5
				Else
					BallShadowA(s).X = bx + BS_d10 + ((bx - TW_d2) / BS_dAM) + offsetX - 5
				End If
				BallShadowA(s).Y = by + BS_d10 + offsetY
		...
		If DynamicBallShadowsOn Then
			If bz < 30 And bx < 850 Then
				...
				If dist1 < falloff Then
					objrtx1(s).visible = 1 : objrtx1(s).X = bx : objrtx1(s).Y = by
				...
				If dist2 < falloff Then
					objrtx2(s).visible = 1 : objrtx2(s).X = bx : objrtx2(s).Y = by + offsetY
...
sub TargetBouncer(aBall,defvalue)
    ...
    if TargetBouncerEnabled = 1 and aball.z < 30 then
        ...
        Dim vr2 : vr2 = vratio * vratio
        aBall.velx = sgn(aBall.velx) * sqr(abs((vel*vel - vz2)/(1+vr2)))
...
Sub FlipperNudge(fBOT, Flipper1, Endangle1, EOSNudge1, Flipper2, EndAngle2)
	...
	If ca1 = Endangle1 and EOSNudge1 <> 1 Then
		...
		If Flipper2.currentangle = EndAngle2 Then
			Dim ub : ub = UBound(fBOT)
			Dim bx, by
			For b = 0 to ub
				bx = fBOT(b).x : by = fBOT(b).y
...
' Pre-computed constants for DynamicBSUpdate
...
' Pre-computed audio constants
Dim InvTWHalf : InvTWHalf = 2 / tablewidth
Dim InvTHHalf : InvTHHalf = 2 / tableheight

' Pre-built string arrays for rolling sounds
Dim BallRollStr(11)
Dim brsI : For brsI = 0 To 11 : BallRollStr(brsI) = "BallRoll_" & brsI : Next

' Pre-built string arrays for ramp sounds
...
' Flipper shadow tracking for write guards
...
Function FlipperTrigger(ballx, bally, Flipper)
	Dim fx, fy, fca, flen, frad, DiffAngle
	...
	If DistancePL(ballx,bally,fx,fy,Cos(frad)+fx,Sin(frad)+fy) < 48 and DiffAngle <= 90 and Distance(ballx,bally,fx,fy) < flen Then
		FlipperTrigger = True
	Else
...
Sub FlipperDeactivate(Flipper, FlipperPress)
	...
	If Abs(Flipper.currentangle) <= Abs(Flipper.endangle) + 0.1 Then
		...
		For b = 0 to UBound(gBOT)
			If Distance(gBOT(b).x, gBOT(b).y, fx, fy) < 55 Then
...
Class CoRTracker
	...
	Public Sub Update()
		Dim b, BOT, bid : BOT = GetBalls
		If UBound(BOT) = -1 Then Exit Sub
		For b = 0 To UBound(BOT)
			bid = BOT(b).id
			If bid <= tnob Then
				ballvel(bid) = BallSpeed(BOT(b))
				ballvelx(bid) = BOT(b).velx
				ballvely(bid) = BOT(b).vely
			End If
		Next
	End Sub
End Class
...
Sub RollingUpdate()
	...
	For b = 0 to ub
		...
		If AmbientBallShadowOn = 0 Then
			...
			End If
			BallShadowA(b).Y = gBOT(b).Y + BS_d5 + offsetY
			BallShadowA(b).X = gBOT(b).X + offsetX
			BallShadowA(b).visible = 1
		End If
	Next
...
Sub RampRollUpdate()		'Timer update
	dim x : for x = 1 to uBound(RampBalls)
		if Not IsEmpty(RampBalls(x,1) ) then
			if BallVel(RampBalls(x,0) ) > 1 then
				If RampType(x) then
			...
			if RampBalls(x,0).Z < 30 and RampBalls(x, 2) > RampMinLoops then
				StopSound(RampLoopStr(x))
				StopSound(WireLoopStr(x))
...
Function Vol(ball)
	Dim bv : bv = BallVel(ball) : Vol = Csng(bv * bv)
End Function
